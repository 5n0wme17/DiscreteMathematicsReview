...
考虑 xⁿ = xⁿ⁻¹ + xⁿ⁻²，其中 x ≠ 0。有两个不同的根
φ = (1+√5)/2, ψ = (1-√5)/2

那么 Fₙ 可以形式为 aφⁿ + bψⁿ。
由 F₀ = 0 和 F₁ = 1，我们有 a + b = 0 和 φa + ψb = 1，得到 a = 1/√5, b = -a。
因此，Fₙ = (φⁿ - ψⁿ)/√5
...
**定理** 如果这个CE有2个根 r₁ ≠ r₂，那么序列{aₙ}是递推关系的解当且仅当
aₙ = α₁r₁ⁿ + α₂r₂ⁿ 对于 n ≥ 0 和常数 α₁, α₂
...
**定理** 如果这个CE有k个不同的根 rᵢ，那么递推关系的解形式为
aₙ = ∑ᵢ₌₁ᵏ αᵢrᵢⁿ 对于所有 n ≥ 0，其中 αᵢ 是常数

**示例** aₙ = 2aₙ₋₁ + 5aₙ₋₂ - 6aₙ₋₃

## 退化根的情况

**定理** 如果CE r² - c₁r - c₂ = 0 只有一个根 r₀，那么
aₙ = α₁r₀ⁿ + α₂nr₀ⁿ，对于所有 n ≥ 0 和两个常数 α₁ 和 α₂
...
**定理**[定理4, p.519] 假设有t个根 r₁, ..., rₜ 具有重数 m₁, ..., mₜ。那么
aₙ = ∑ᵢ₌₁ᵗ (∑ⱼ₌₀ᵐⁱ⁻¹ αᵢ,ⱼnʲ)rᵢⁿ，对于所有 n ≥ 0 和常数 αᵢ,ⱼ

**示例** aₙ = -3aₙ₋₁ - 3aₙ₋₂ - aₙ₋₃ 其中 a₀ = 1, a₁ = -2, a₂ = -1

## 线性非齐次递推关系

**定义** 具有常数系数的线性非齐次关系可能包含一些仅依赖于n的项 F(n)
...
**定理** 如果 aₙ = p(n) 是具有常数系数的线性非齐次关系
aₙ = c₁aₙ₋₁ + c₂aₙ₋₂ + ··· + cₖaₙ₋ₖ + F(n)
的任何特解，那么它的所有解形式为 aₙ = p(n) + h(n)，其中 aₙ = h(n) 是相关齐次递推关系 aₙ = c₁aₙ₋₁ + c₂aₙ₋₂ + ··· + cₖaₙ₋ₖ 的任何解。
...
设 p(n) = cn + d，那么
cn + d = 3(c(n-1) + d) + 2n，这意味着 (2c+2)n + (2d-3c) = 0
...
有限序列 a₀, a₁, ..., aₙ 可以通过设 aₙ₊₁ = aₙ₊₂ = ··· = 0 轻松扩展

这个无穷序列 {aₙ} 的生成函数 G(x) 是n次多项式，即
G(x) = a₀ + a₁x + ··· + aₙxⁿ
...
集合中的n个元素每个都对生成函数 f(x) = ∑ₖ₌₀ⁿ aₖxᵏ 贡献项 (1+x)。因此，f(x) = (1+x)ⁿ。然后由二项式定理，我们有 aₖ = (n选k)
